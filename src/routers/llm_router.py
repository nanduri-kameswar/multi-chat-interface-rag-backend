...
from src.routers.routers_deps import DocumentService_Dependency, LLMService_Dependency, MessageService_Dependency
...
async def query(
    ...
    recent_k_messages: list[MessageResponse] | None = await message_service.get_recent_k_conversation_messages(payload.conversation_id)
    chat_history: list[dict[str, Any]] = convert_messages_to_chat_history(recent_k_messages) if recent_k_messages else []
    # add human message to db
    human_message = MessageCreate.model_validate({payload.conversation_id, MessageRole.USER, payload.query})
    ...
    assistant_message = MessageCreate.model_validate({payload.conversation_id, MessageRole.ASSISTANT, result.answer})
